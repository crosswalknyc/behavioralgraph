"""Re-export shim -> migration.post_generation_enforcers (TWIN_SYNC_SHIM_V1).
...
``from post_generation_enforcers import ...``), silently skipping newer logic.

The maintained implementation lives at ``migration/post_generation_enforcers.py``,
byte-synced from the parent repo's ``migration/post_generation_enforcers.py``.
...
import migration.post_generation_enforcers as _impl
