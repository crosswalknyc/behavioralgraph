"""Re-export shim -> migration.crosswalk_audit_framework (TWIN_SYNC_SHIM_V1).
...
``from crosswalk_audit_framework import ...``), silently skipping newer logic.

The maintained implementation lives at ``migration/crosswalk_audit_framework.py``,
byte-synced from the parent repo's ``migration/crosswalk_audit_framework.py``.
...
import migration.crosswalk_audit_framework as _impl
