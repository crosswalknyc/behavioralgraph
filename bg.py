...
def ai_host_demographic_review(df, brand_category, project_name, brands):
    """GPT-4o powered demographic review for HOST/PERSONALITY profiles.

    Covers a wide range of public figures: late night hosts, celebrity chefs,
    business/investor personalities, models/fashion icons, comedians,
    science/education figures, motivational speakers, and reality TV stars.
    """
    bc_upper = (brand_category or '').strip().upper()
    if 'HOST' not in bc_upper and 'PERSONALITY' not in bc_upper:
        return df

    client = _get_openai_client()
    if not client:
        print("⚠️  OpenAI not available — skipping host/personality demographic review")
        return df
...
def ai_musician_demographic_review(df, brand_category, project_name, brands):
    """GPT-4o powered demographic review for MUSICIAN/BAND profiles.

    Covers solo artists, bands, and musical groups across all genres:
    pop, hip-hop, country, rock, metal, K-pop, Latin/reggaeton, EDM,
    R&B/soul, indie, folk, and legacy/deceased acts.
    """
    bc_upper = (brand_category or '').strip().upper()
    if 'MUSICIAN' not in bc_upper and 'BAND' not in bc_upper:
        return df

    client = _get_openai_client()
    if not client:
        print("⚠️  OpenAI not available — skipping musician demographic review")
        return df
...
{key_block}
If accurate (within 3%): {{"status":"OK","notes":"reason"}}
If corrections needed: {{"status":"FIX","notes":"what's wrong","corrections":{{"CAT":{{"label":num,...}},...}}}}
Each corrected category MUST sum to 100. JSON only, no markdown."""
...
def ai_politics_demographic_review(df, brand_category, project_name, brands):
    """GPT-4o powered demographic review for POLITICS/ACTIVIST profiles.

    Covers politicians, activists, and political figures across the
    political spectrum with party-specific audience reasoning.
    """
    bc_upper = (brand_category or '').strip().upper()
    if 'POLITIC' not in bc_upper and 'ACTIVIST' not in bc_upper:
        return df

    client = _get_openai_client()
    if not client:
        print("⚠️  OpenAI not available — skipping politics demographic review")
        return df
...
def ai_creative_demographic_review(df, brand_category, project_name, brands):
    """GPT-4o powered demographic review for WRITER/DIRECTOR/AUTHOR/ARTIST profiles.

    Covers filmmakers, showrunners, screenwriters, authors, and visual artists
    with genre-specific audience reasoning.
    """
    bc_upper = (brand_category or '').strip().upper()
    if not any(t in bc_upper for t in ('WRITER', 'DIRECTOR', 'AUTHOR', 'ARTIST')):
        return df

    client = _get_openai_client()
    if not client:
        print("⚠️  OpenAI not available — skipping creative demographic review")
        return df
...
def ai_media_demographic_review(df, brand_category, project_name, brands):
    ...
    Reasons about the READER/USER BASE of each digital media publication —
    who actually reads, clicks, and subscribes to the outlet's digital content
    (website, app, newsletters, social feeds) — based on editorial identity,
    content focus, political lean, and known audience composition from
    Pew Research, Comscore, Reuters Digital News Report, and similar sources.
    """
    bc_upper = (brand_category or '').strip().upper()
    if bc_upper != 'MEDIA':
        return df
    ...
    client = _get_openai_client()
    if not client:
        print("⚠️  OpenAI not available — skipping media demographic review")
        return df
...
Also note: owner/parent company, editorial lean (if political), signature content, and digital platform presence (website, app, newsletter, social).
