...
def load_keys(path: Optional[str] = None) -> List[str]:
    ...
    if path:
        ...
        # Allow callers (e.g. a parallel avid rerun running alongside an
        # AA-cuts run that already owns the first N keys) to point this
        # process at a SUBSET file via AVID_POOL_PATH, without disturbing
        # the canonical .env.avid_skins. Added 2026-06-16.
        env_override = os.environ.get("AVID_POOL_PATH", "").strip()
