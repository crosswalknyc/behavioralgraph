...
def synthesize_audience_cut(
    ...
    *,
    ...
    # MANDATORY write-time safety net (2026-08-06). Idempotent: fills
    # blank BP from Raw, strips % from BP/CS, recomputes Raw/Proj,
    # recomputes non-demo Category Share to sum to 100, blanks stale
    # meta-row CS. Same guarantee that write_profile_csv provides on
    # the main pipeline path -- audience_cut_synthesis writes direct
    # to S3 so we invoke the safety net inline here.
    try:
        try:
        ...
        df_cut, _sn_stats = run_write_safety_net(
            df_cut, subject, verbose=True,
        )
