"""Re-export shim -> migration.avid_key_pool (TWIN_SYNC_SHIM_V1).
...
``from avid_key_pool import ...``), silently skipping newer logic.

The maintained implementation lives at ``migration/avid_key_pool.py``,
byte-synced from the parent repo's ``migration/avid_key_pool.py``.
...
import migration.avid_key_pool as _impl
