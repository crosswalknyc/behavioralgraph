"""
HBO Max on Amazon: HBO Max carried on Amazon Prime Video Channels.
...
THE CATALOG IS THE SAME. HBO Max sold through Prime Video Channels is
the same entitlement and the same title list as the HBO Max app, so
this module scrapes nothing. It mirrors `latest/max.json` under
...
mdc`). HBO Max publishes no Amazon split, so the share is a BAND
bracketed from the published quantities around the service; the band,
the anchors and the working live in
`scripts/trends_scrapers/carriage_mix.py` under `max`, and the
registry entry that applies it lives in
`scripts/trends_scrapers/derived_rails.py` under `max_amazon`.
...
the originals people install the HBO Max app for sit low, and the
...
HOW THE NUMBER IS PRODUCED. This panel is a SUBSET of the HBO Max
panel, so its number is computed from that title's HBO Max number at
the point the board is produced, never carried alongside it.

Standalone:
    python3 -m scripts.trends_scrapers.max_amazon
"""
...
def amazon_share_for_title(title: str, category_display: str = '',
                           day_iso: str = '') -> float:
    """The share of this title's HBO Max audience carried through
