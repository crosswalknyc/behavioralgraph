"""
MGM+ on Amazon: MGM+ carried on Amazon Prime Video Channels.
...
THE CATALOG IS THE SAME. MGM+ sold through Prime Video Channels is
the same entitlement and the same title list as the MGM+ app, so
this module scrapes nothing. It mirrors `latest/mgmplus.json` under
...
mdc`). MGM+ publishes no Amazon split, so the share is a BAND
bracketed from the published quantities around the service; the band,
the anchors and the working live in
`scripts/trends_scrapers/carriage_mix.py` under `mgmplus`, and the
registry entry that applies it lives in
`scripts/trends_scrapers/derived_rails.py` under `mgmplus_amazon`.
...
the originals people install the MGM+ app for sit low, and the
...
HOW THE NUMBER IS PRODUCED. This panel is a SUBSET of the MGM+
panel, so its number is computed from that title's MGM+ number at
the point the board is produced, never carried alongside it.

Standalone:
    python3 -m scripts.trends_scrapers.mgmplus_amazon
"""
...
def amazon_share_for_title(title: str, category_display: str = '',
                           day_iso: str = '') -> float:
    """The share of this title's MGM+ audience carried through
