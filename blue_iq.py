...
def blue_iq_cache_key(filters: dict) -> str:
    ...
    History:
     ...
     v27 — 2026-07-27: `top_searches` card retired on the frontend
            per Jenna ("kill this block"). Backend now emits an empty
            list for that field to save OpenAI synth cost and the
            shape/dampen loops, keeping the field present so
            downstream consumers don't KeyError. Issue-buckets card
            expands to full row width in place of the pair. Bumping
...
def compute_panel_view(filters: dict, *, force_refresh: bool = False) -> dict:
    ...
    # `top_searches` card was retired on the frontend 2026-07-27 per
    # Jenna. We keep the field in the payload (empty list) so any
    # downstream/API consumers reading it don't KeyError, but we no
    # longer pay the OpenAI synth cost or the extra shape/dampen
    # loops. The upstream `panel_top_queries` fetch stays because
    # it feeds the AI-bucketed "Political issue searches" card, the
    # journey mapper, and the issue-bucket sample-query evidence.
    # `_shape_top_searches`, `_flag_political_term`, and
    # `synthesize_top_searches` in blue_iq_synth_agent.py remain in
    # place as dead code — they cost nothing at rest and would be
    # the natural entry points if the card is ever reintroduced.
    top_searches_out: list[dict] = []
