...
YOUTUBE_RANKER_LOCK = threading.Lock()
YOUTUBE_RANKER_CACHE_FILE = os.path.join(os.path.dirname(__file__), 'youtube_ranker_cache.json')
YOUTUBE_RANKER_CACHE_MAX_AGE_HOURS = 24

def _build_youtube_ranker_payload(rows):
    """Build API payload from raw rows: (visit_date, title, channel, views)."""
    from collections import defaultdict
    daily = []
    by_date = defaultdict(list)
    ...
    for r in rows:
        dt = r[0].strftime('%Y-%m-%d') if hasattr(r[0], 'strftime') else str(r[0])[:10]
        title = (r[1] or '').strip() or 'Unknown'
        channel = (r[2] or '').strip() or '-'
        views = int(r[3] or 0)
        daily.append({'date': dt, 'title': title, 'channel': channel, 'views': views})
        by_date[dt].append({'title': title, 'channel': channel, 'views': views})
        by_title[title].append({'date': dt, 'views': views})
    dates_sorted = sorted(by_date.keys())
    date_range = {'min': dates_sorted[0], 'max': dates_sorted[-1]} if dates_sorted else {}
    channels_by_date = defaultdict(list)
    for dt, items in by_date.items():
        ch_views = defaultdict(lambda: {'views': 0, 'top_title': '', 'top_views': 0})
        for it in items:
            ch = it['channel'] or 'Other'
            ch_views[ch]['views'] += it['views']
            if it['views'] > ch_views[ch]['top_views']:
                ch_views[ch]['top_title'] = it['title']
                ch_views[ch]['top_views'] = it['views']
        for ch, v in ch_views.items():
            channels_by_date[dt].append({'channel': ch, 'views': v['views'], 'top_title': v['top_title']})
    title_totals = [(name, sum(p['views'] for p in pts)) for name, pts in by_title.items()]
    title_totals.sort(key=lambda x: -x[1])
    top_20_titles = [s[0] for s in title_totals[:20]]
    top_titles_over_time = []
    for name in top_20_titles:
        pts = by_title[name]
        by_date_title = {p['date']: p['views'] for p in pts}
        top_titles_over_time.append({'title': name, 'by_date': by_date_title})
    return {
        'daily': daily,
        'by_date': dict(by_date),
        'by_title': dict(by_title),
        'channels_by_date': dict(channels_by_date),
        'top_titles_over_time': top_titles_over_time,
        'dates_sorted': dates_sorted,
        'date_range': date_range
    }

def _fetch_youtube_ranker_from_snowflake(refresh_today_only=False):
    """Query BEHAVIORALGRAPH.YOUTUBE.YOUTUBE for past year (or latest day only)."""
    try:
        import bg
        conn = bg.connect_snowflake()
        cur = conn.cursor()
    ...
    try:
        if refresh_today_only:
            sql = """
                SELECT DATE(VISIT_TS) AS visit_date, TITLE, CHANNEL, COUNT(*) AS views
                FROM BEHAVIORALGRAPH.YOUTUBE.YOUTUBE
                WHERE VISIT_TS >= CURRENT_DATE()
        ...
        cur.execute(sql)
        rows = cur.fetchall()
        return _build_youtube_ranker_payload(rows)
    finally:
        try:
            conn.close()
        except Exception:
...
def _load_youtube_ranker_cache():
    global YOUTUBE_RANKER_CACHE
    if not os.path.exists(YOUTUBE_RANKER_CACHE_FILE):
        return None
    try:
        with open(YOUTUBE_RANKER_CACHE_FILE, 'r') as f:
            data = json.load(f)
        YOUTUBE_RANKER_CACHE['data'] = data
        YOUTUBE_RANKER_CACHE['loaded_at'] = datetime.now().timestamp()
        return data
    except Exception:
        return None

def _save_youtube_ranker_cache(data):
    try:
        with open(YOUTUBE_RANKER_CACHE_FILE, 'w') as f:
            json.dump(data, f, indent=0)
    except Exception as e:
...
def get_youtube_ranker_data():
    ...
    global YOUTUBE_RANKER_CACHE
    refresh_today = request.args.get('refresh_today', '').lower() in ('1', 'true', 'yes')
    with YOUTUBE_RANKER_LOCK:
        data = YOUTUBE_RANKER_CACHE.get('data')
        loaded_at = YOUTUBE_RANKER_CACHE.get('loaded_at') or 0
        age_hours = (datetime.now().timestamp() - loaded_at) / 3600.0
        if data is None:
            data = _load_youtube_ranker_cache()
        if data is not None and age_hours >= YOUTUBE_RANKER_CACHE_MAX_AGE_HOURS:
            refresh_today = True
        if data is None or (refresh_today and data.get('date_range')):
            try:
                if data and refresh_today and data.get('date_range'):
                    today_payload = _fetch_youtube_ranker_from_snowflake(refresh_today_only=True)
                    by_date = data.get('by_date', {})
                    for dt, rows in (today_payload.get('by_date') or {}).items():
                        by_date[dt] = rows
                    data['by_date'] = by_date
                    ...
                    for title_name, pts in (today_payload.get('by_title') or {}).items():
                        by_title.setdefault(title_name, []).extend(pts)
                    data['by_title'] = by_title
                    _prev_dates = data.get('dates_sorted', [])
                    _new_dates = list((today_payload.get('by_date') or {}).keys())
                    dates_sorted = sorted(set(_prev_dates + _new_dates))
                    data['dates_sorted'] = dates_sorted
                    if dates_sorted:
                        data['date_range'] = {'min': dates_sorted[0], 'max': dates_sorted[-1]}
                    channels_by_date = data.get('channels_by_date', {})
                    for dt, rows in (today_payload.get('channels_by_date') or {}).items():
                        channels_by_date[dt] = rows
                    data['channels_by_date'] = channels_by_date
                    by_title = data['by_title']
                    title_totals = [(name, sum(p['views'] for p in pts)) for name, pts in by_title.items()]
                    title_totals.sort(key=lambda x: -x[1])
                    top_20_titles = [s[0] for s in title_totals[:20]]
                    top_titles_over_time = []
                    for name in top_20_titles:
                        pts = by_title[name]
                        top_titles_over_time.append({'title': name, 'by_date': {p['date']: p['views'] for p in pts}})
                    data['top_titles_over_time'] = top_titles_over_time
                else:
                    data = _fetch_youtube_ranker_from_snowflake(refresh_today_only=False)
                YOUTUBE_RANKER_CACHE['data'] = data
                YOUTUBE_RANKER_CACHE['loaded_at'] = datetime.now().timestamp()
                _save_youtube_ranker_cache(data)
            except Exception as e:
                if data is None:
                    return jsonify({'error': str(e)}), 500
    return jsonify(data or {})
