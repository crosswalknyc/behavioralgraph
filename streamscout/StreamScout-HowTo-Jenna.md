...
- **Buy** → **Amazon** (Kindle *and* print — paperback/hardcover), plus
  content-map proxy terms for **Bookshop.org**, **Target / Books-A-Million**,
  **Walmart**, **Barnes & Noble**, and **Costco**.
- **Listen** → **Audible**, **Apple Books** audiobook, and the **Amazon**
  Audible Audio Edition.
- **Borrow** → **Libby / OverDrive** holds (`overdrive.com/media/<id>`).

A **franchise** rolls up into **one SHOW** with `SEASON = Book N` — e.g. the
five *Merciless Saints* books all land under `SHOW = Merciless Saints`, Books
1–5. Run it standalone for a series like this:

```bash
python3 streamscout/books_identifier.py --franchise "Merciless Saints" \
  --titles "Merciless Saints" "Cruel Saints" "Ruthless Saints" \
           "Tears of Betrayal" "Tears of Salvation" --author "Michelle Heard"
```

> **Why some rows are short "proxy" terms, not full links.** A few physical
> retailers (Target, BAM, Walmart, B&N, Costco) hard-block automated visits, so
> instead of a fragile scrape Books emits a deterministic **content-map term**
> (`p/merciless-saints`, `ip/merciless-saints`, `w/merciless-saints`,
> `books/merciless-saints`, `merciless-saints.product`). These ride in the
> **content map** exactly like the real URLs. Because it's the content map, the
