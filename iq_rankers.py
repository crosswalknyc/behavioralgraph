...
# ============================================================================
# Profile Context — "what's actually driving today's mentions?"
# ============================================================================


def compute_profile_context(
    *,
    ch_connect: Callable,
    profile_subject: str,
    brand_terms: list[str],
    ...
    max_urls: int = 30,
    max_common_names: int = 15,
    max_domains: int = 10,
    max_path_clusters: int = 12,
) -> dict[str, Any]:
    """Pull the URL surface that's mentioning a profile in [start, end] and
    bucket it into the rough shapes a human cares about: top common_names,
    top domains, top URL-path stems, and a deduped set of representative
    URLs to send to an LLM for a narrative rollup.

    All counts come straight out of clickstream.clickstream_final via the
    same `multiSearchAny(lower(URL/COMMON_NAME), brand_terms)` filter the
    daily compute uses, so context numbers reconcile with the leaderboard
    Mentions column.

    The LLM rollup itself is generated by app.py — keeping the OpenAI
    dependency out of this module so iq_rankers stays independently
    importable for the cron and backfill scripts.
    """
    out: dict[str, Any] = {
        "profile_subject": profile_subject,
        "start": start,
    ...
    term_lit = _term_array_literal(brand_terms or [])
    if not term_lit:
        out["warnings"].append("no brand_terms — cannot filter clickstream")
        return out

    where_filter = (
        f"DELIVERED BETWEEN toDate('{start}') AND toDate('{end}') "
        f"AND length(URL) > 5 "
        f"AND (multiSearchAny(lower(URL), [{term_lit}]) "
        f"     OR multiSearchAny(lower(COMMON_NAME), [{term_lit}]))"
    )

    # Single CH connection for all four queries — cheaper than re-opening
    # the HTTP keepalive per query.
    conn = ch_connect(settings={"max_execution_time": 120, "use_skip_indexes": 1})
    try:
        cur = conn.cursor()

        # 1) Top COMMON_NAMES — the cleanest "what is this URL about" signal.
        cur.execute(f"""
            SELECT lower(coalesce(COMMON_NAME, '')) AS cn,
                   count() AS hits,
                   uniqExact(UID) AS uniq
            FROM clickstream.clickstream_final
            WHERE {where_filter}
              AND length(coalesce(COMMON_NAME, '')) > 0
            GROUP BY cn
            ORDER BY hits DESC
            LIMIT {int(max_common_names)}
        """)
        out["top_common_names"] = [
            {"common_name": r[0], "hits": int(r[1]), "uniq": int(r[2])}
            for r in (cur.fetchall() or [])
        ]

        # 2) Top DOMAINS — gives the platform shape (which streamer, which
        #    social, which news outlet is driving the talk).
        cur.execute(f"""
            SELECT lower(coalesce(DOMAIN, '')) AS d,
                   count() AS hits,
                   uniqExact(UID) AS uniq
            FROM clickstream.clickstream_final
            WHERE {where_filter}
              AND length(coalesce(DOMAIN, '')) > 0
            GROUP BY d
            ORDER BY hits DESC
            LIMIT {int(max_domains)}
        """)
        out["top_domains"] = [
            {"domain": r[0], "hits": int(r[1]), "uniq": int(r[2])}
            for r in (cur.fetchall() or [])
        ]

        # 3) URL path clusters — strip query / fragment / trailing-id, group
        #    by the human-readable stem so /title/meet-the-parents and
        #    /title/meet-the-parents?ref=foo collapse to the same row.
        cur.execute(f"""
            WITH cleaned AS (
                SELECT
                    lower(extract(URL,
                        '^https?://[^/?#]+(/[^?#]*)?')) AS path,
                    lower(coalesce(DOMAIN, '')) AS d,
                    UID
                FROM clickstream.clickstream_final
                WHERE {where_filter}
            )
            SELECT d, path, count() AS hits, uniqExact(UID) AS uniq
            FROM cleaned
            WHERE length(path) > 1
            GROUP BY d, path
            ORDER BY hits DESC
            LIMIT {int(max_path_clusters)}
        """)
        out["top_path_clusters"] = [
            {"domain": r[0], "path": r[1], "hits": int(r[2]), "uniq": int(r[3])}
            for r in (cur.fetchall() or [])
        ]

        # 4) Sample raw URLs for the LLM. We dedupe on the truncated URL so
        #    one viral page can't crowd out the rest of the surface, and
        #    pull the highest-traffic distinct URLs first.
        cur.execute(f"""
            SELECT URL, lower(coalesce(COMMON_NAME, '')) AS cn,
                   lower(coalesce(DOMAIN, '')) AS d,
                   count() AS hits
            FROM clickstream.clickstream_final
            WHERE {where_filter}
        ...
        """)
        out["sample_urls"] = [
            {"url": r[0], "common_name": r[1], "domain": r[2], "hits": int(r[3])}
            for r in (cur.fetchall() or [])
        ]

        # 5) Headline totals so the modal can show "X mentions / Y panelists
        #    drove this rollup".
        cur.execute(f"""
            SELECT count() AS hits, uniqExact(UID) AS uniq
            FROM clickstream.clickstream_final
            WHERE {where_filter}
        """)
        row = (cur.fetchall() or [[0, 0]])[0]
        out["total_mentions"]   = int(row[0] or 0)
        out["unique_panelists"] = int(row[1] or 0)

    except Exception as e:
        out["warnings"].append(f"clickstream query failed: {str(e)[:200]}")
    finally:
        try:
            conn.close()
        except Exception:
