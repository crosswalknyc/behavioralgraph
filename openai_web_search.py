"""Re-export shim -> migration.openai_web_search (TWIN_SYNC_SHIM_V1).
...
``from openai_web_search import ...``), silently skipping newer logic.

The maintained implementation lives at ``migration/openai_web_search.py``,
byte-synced from the parent repo's ``migration/openai_web_search.py``.
...
import migration.openai_web_search as _impl
